...
public class DrawableBase implements Drawable {
	...
	private static FloatBuffer put(Object floats, FloatBuffer buf) {
		if (floats instanceof float[][]) {
			...
			throw new IllegalArgumentException("Every data pair must end with "+float[][][].class + " or "+ float[][].class);
		}
		return buf;
	}
	...
	/**
	 * @param drawMode e.g, GL11.GL_TRIANGLES, GL11.GL_TRIANGLE_STRIP, GL11.GL_TRIANGLE_FAN
	 * @param indices vertex indices
	 * @param attribs Sequence of (Attribute, float[][]) or (Attribute, float[][][])
	 */
	...
	public void init(int drawMode, Object... attribs) {
		...
		for (int i = 0; i < attribCount; i++) {
			...
			if (vertexCount < 0) {
				...
				throw new RuntimeException("all attribute arrays must specify same number of attribute values (= number of vertices)");
			}
